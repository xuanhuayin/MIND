...
本版仅在你的原始脚本上新增：
- --only_list 选项（默认指向 TRIBE_8features/video_2hz/bad_video_ids.txt）
- 若提供/存在该文件，只处理其中列出的 dataset ID（每行一个 stem）
"""
...
def open_video(path: Path) -> tuple[cv2.VideoCapture, float, int]:
    ...
    nframes = int(cv2.CAP_PROP_FRAME_COUNT and cap.get(cv2.CAP_PROP_FRAME_COUNT) or 0)
...
def main():
    ...
    bad_list_path = Path(args.only_list) if args.only_list else DEFAULT_BAD_LIST
    if bad_list_path.exists():
        only_set = {ln.strip() for ln in open(bad_list_path, "r", encoding="utf-8") if ln.strip()}
        print(f"[FILTER] Only processing {len(only_set)} IDs from {bad_list_path}")
