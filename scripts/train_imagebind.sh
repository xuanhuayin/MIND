#!/usr/bin/env bash
# Train MIND with ImageBind backbone features.
# ImageBind features: 1 layer, 1024-dim per modality.
set -euo pipefail
...
python train.py \
    ...
    --video_root "${DATA_ROOT}/pipeline_IMAGEBIND/features/video_2hz/sub-01" \
    --text_root  "${DATA_ROOT}/pipeline_IMAGEBIND/features/text_2hz/sub-01" \
    --audio_root "${DATA_ROOT}/pipeline_IMAGEBIND/features/audio_2hz/sub-01" \
    ...
    --moe_aux_weight 0.01 \
    --subject_embedding \
    --out_dir outputs/MIND_ImageBind \
    --log_dir logs/MIND_ImageBind \
    "$@"
