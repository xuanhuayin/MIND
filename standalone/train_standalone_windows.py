...
def resolve_train_probe_ds(train_ids: List[str], epcode: str) -> str:
    ...
    """
    epi = epcode.lower()
    pat = re.compile(rf"task-({epi})$", flags=re.IGNORECASE)
    for ds in train_ids:
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if not np.isnan(val_pearson_mean):
        ...
        # B) TRAIN-PROBE 提升：仅更新 TRAIN-PROBE 可视化（不动 ckpt/VAL）
