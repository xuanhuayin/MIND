...
- Compare ONLY full episodes (no window plots).
- Every epoch:
  * Evaluate full-episode Pearson on VAL set (reconstructed by overlap-average)
  * Evaluate full-episode Pearson on ONE TRAIN episode (train-probe) you specify
  * Log both metrics to TensorBoard and print
  * If VAL improves best: save ckpt + run visualization script for:
      - best VAL episode (or --vis_val_ds_override if present and available)
  * If TRAIN-PROBE improves best: run visualization script ONLY for train-probe
"""
...
def ds2friends(ds: str) -> str:
    """'ses-001_task-s01e02a' -> 'friends_s01e02a'  (for fmri filename)"""
...
def eval_full_episode(
    ...
    # 取整段 GT
    friends_key = ds2friends(ds)
    fmri_path = fmri_root / f"{friends_key}_fmri.npy"
    # ^^^ 修正：去掉这行末尾的多余括号如果你的编辑器提示语法错误
    # 正确写法如下（已在下方使用）：
    fmri_path = fmri_root / f"{friends_key}_fmri.npy"
...
def main():
    ...
    ap.add_argument("--vis_train_ds_override", type=str, default="ses-001_task-s01e02a",
                    help="必须是 train_list 里的一个 dataset id；用它来做整段的 train-probe 指标与可视化")
    ...
    (out_dir / "preds_val_windows").mkdir(parents=True, exist_ok=True)   # 仍需中间目录，但不再作图
    ...
    if args.vis_train_ds_override not in set(train_ids):
        raise SystemExit(f"--vis_train_ds_override '{args.vis_train_ds_override}' 不在 train_list 里。")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            ...
            for batch in pbar_v:
                ...
                # collect (for global pearson on windows)
        ...
        val_pearson_mean = float(np.nanmean(pearson))
        ...
        for ds, acc in recon_pred.items():
            ...
            friends_key = ds2friends(ds)
            gt_all = np.load(Path(args.fmri_root) / f"{friends_key}_fmri.npy")
            ...
            # choose the longest episode as default best for visualization
            if T > best_len:
                best_len = T
                best_ds = ds
        ...
        # train-probe Pearson (full episode)
        ...
        writer.add_scalar("metric/val_pearson_mean", float(val_pearson_mean), epoch)
        ...
        print(f"Epoch {epoch}: "
              ...
              f"val_pearson_mean={val_pearson_mean:.6f}  "
              f"trainprobe_pearson_mean={trainprobe_pearson_mean:.6f}")

        # --------------- Save best & visualize (FULL EPISODES ONLY) ---------------

        # A) VAL 指标创新高：只更新 VAL 的可视化与（沿用原行为）checkpoint
            ...
            if best_val_ds_for_vis is not None:
                ...
                if args.print_vis_shapes:
                    try:
                        ...
                        print(f"[VIS] VAL FULL ds={ds} GT={ep_gt_path} shape={_gt.shape}  PRED={ep_pred_path} shape={_pr.shape}")
        ...
        # B) TRAIN-PROBE 指标创新高：只更新 TRAIN-PROBE 的可视化（不影响 VAL 可视化与 checkpoint）
        if trainprobe_pearson_mean > best_trainprobe:
            ...
            if args.print_vis_shapes:
                try:
                    ...
                    print(f"[VIS] TRAIN-PROBE FULL ds={train_probe_ds} GT={tp_gt_path} shape={_gtp.shape}  PRED={tp_pred_path} shape={_prp.shape}")
                except Exception as e:
