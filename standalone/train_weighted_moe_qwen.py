...
class WindowedDataset(Dataset):
    def __init__(self, ids: List[str], video_root: Path, text_root: Path, audio_root: Path,
        ...
        # === 兼容 [T,L,D] / [T,D]：用统一的读取函数探测层数 ===
        v0_LDT = self._load_feature_LDT(self.video_root / f"{ids[0]}.npy")  # 返回 [L,D,T]
        probe_L = v0_LDT.shape[0]  # 这里就是层/组维
    ...
    # === 新增：把模态的 [G,D,T] 与 self.G 对齐（1->G 时重复，多于 G 时截断）===
    ...
    def __getitem__(self, i: int) -> Dict[str, torch.Tensor]:
        ...
        for name, root in (("video", self.video_root), ("text", self.text_root), ("audio", self.audio_root)):
            lat_LDT = self._load_feature_LDT(root / f"{ds}.npy")  # [L,D,T] 或 [1,D,T]
            lat_GDT = self._maybe_pick_layers(lat_LDT)            # 先做 layer 选择/分组
            lat_GDT = self._align_G(lat_GDT)                      # 再对齐到统一的 G
